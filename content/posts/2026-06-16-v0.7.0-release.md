...
- **P/D-disaggregation transfer** — `AIBrixPDReuseConnector` ([#2060](https://github.com/vllm-project/aibrix/issues/2060), [#2092](https://github.com/vllm-project/aibrix/issues/2092)) lets a prefill worker write its KV to the store and a decode worker read it. Disaggregation becomes *just reuse with a single consumer* — no separate point-to-point fabric, and it works **single-node, without RDMA**, on commodity GPUs like the **L20** — validated in our internal testing.
