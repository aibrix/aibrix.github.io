...
* Achieved full support for vLLM v0.9.1's V0 and V1 architectures.
...
* **Label configuration:** add `model.aibrix.ai/engine` to the pod YAML to specify the engine type and use `model.aibrix.ai/name` and related labels to identify the model. For example, mark a vLLM deployment with `engine: "vllm"`
...
* **Extensibility:** to add a new engine, you simply map metric names in the code and declare the engine in the deployment template; AIBrix then adapts to the new metric sources.
